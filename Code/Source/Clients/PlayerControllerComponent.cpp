...
namespace ObjectInteraction
{
    ...
    void PlayerControllerComponent::Reflect(AZ::ReflectContext* rc)
    {
        if (auto sc = azrtti_cast<AZ::SerializeContext*>(rc))
        {
            ...
            if (AZ::EditContext* ec = sc->GetEditContext())
            {
                ...
                ec->Class<PlayerControllerComponent>("Player Controller", "[Player controlled character]")
                    ->ClassElement(ClassElements::EditorData, "")
                    ->Attribute(Attributes::AppearsInAddComponentMenu, AZ_CRC("Game"))
                    ->DataElement(
                        nullptr,
                        &PlayerControllerComponent::m_pitch_sensitivity,
                        "Camera Pitch Rotate Input",
                        "Camera pitch rotation control")
                    ->DataElement(
                        nullptr, &PlayerControllerComponent::m_yaw_sensitivity, "Camera Yaw Rotate Input", "Camera yaw rotation control")
                    ->DataElement(nullptr, &PlayerControllerComponent::m_speed, "Speed", "Player's Speed")
                    ->DataElement(nullptr, &PlayerControllerComponent::m_sprintSpeed, "Sprint Multiplier", "Sprint speed scale factor")
                    ->DataElement(nullptr, &PlayerControllerComponent::m_maxJumpHeight, "Max Jump Height", "Maximum character jump height")
                    ->DataElement(nullptr, &PlayerControllerComponent::m_maxJumpTime, "Max Jump Time", "Time to max jump height")
                    ->DataElement(
                        nullptr, &PlayerControllerComponent::m_fallMultiplier, "Fall Multiplier", "Scale for how fast the character falls")
                    ->DataElement(
                        nullptr,
                        &PlayerControllerComponent::m_shortJump,
                        "Short Jump Multiplier",
                        "Scales gravity to create shorter jump when jump key is released")
                    ->DataElement(
                        nullptr,
                        &PlayerControllerComponent::m_groundCheckRadius,
                        "Ground Check Radius",
                        "Sphere Cast radius used for ground check")
                    ->DataElement(
                        nullptr,
    ...
    void PlayerControllerComponent::handleGravity()
    {
        ...
        Physics::CharacterRequestBus::EventResult(zVelocity, GetEntityId(), &Physics::CharacterRequestBus::Events::GetVelocity);
    ...
    void PlayerControllerComponent::ProcessInput(const float& deltaTime)
    {
        ...
        Physics::CharacterRequestBus::Event(GetEntityId(), &Physics::CharacterRequestBus::Events::AddVelocityForTick, (m_velocity));
