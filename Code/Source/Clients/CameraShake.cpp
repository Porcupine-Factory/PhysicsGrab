...
namespace ObjectInteraction
{
    ...
    void CameraShake::Reflect(AZ::ReflectContext* rc)
    {
        if (auto sc = azrtti_cast<AZ::SerializeContext*>(rc))
        {
            ...
            if (AZ::EditContext* ec = sc->GetEditContext())
            {
                ...
                ec->Class<CameraShake>("Camera Shake", "[Component that shakes the active camera]")
                    ->ClassElement(AZ::Edit::ClassElements::EditorData, "")
                    ->Attribute(Attributes::AppearsInAddComponentMenu, AZ_CRC_CE("Game"))

                    ->DataElement(nullptr, &CameraShake::m_strShake, "Shake Input Key", "Key to initiate or test Camera Shake.")

                    ->DataElement(nullptr, &CameraShake::m_traumaInitial, "Trauma", "Total shake strength")
                    ->DataElement(
                        nullptr,
                        &CameraShake::m_traumaDecay,
                        "Decay",
                        "Adjusts the length of the shake. Value of 0 produces infinite length.")
                    ->DataElement(nullptr, &CameraShake::m_freq, "Speed", "Frequency of the shake")
                    ...
                    ->DataElement(
                        ...
                        "Amplitude of the translational camera shake along X")
                    ->DataElement(
                        nullptr,
                        &CameraShake::m_yTranslationAmplitude,
                        "Y Translation Strength",
                        "Amplitude of the translational camera shake along Y")
                    ->DataElement(
                        nullptr,
                        &CameraShake::m_zTranslationAmplitude,
                        "Z Translation Strength",
                        "Amplitude of the translational camera shake along Z")
                    ...
                    ->DataElement(
                        ...
                        "X Rotation Strength",
                        "Amplitude/Maximum Angle of the rotational camera shake about X")
                    ->DataElement(
                        nullptr,
                        &CameraShake::m_yRotationAmplitude,
                        "Y Rotation Strength",
                        "Amplitude/Maximum Angle of the rotational camera shake about Y")
                    ->DataElement(
                        nullptr,
                        &CameraShake::m_zRotationAmplitude,
                        "Z Rotation Strength",
                        "Amplitude/Maximum Angle of the rotational camera shake about Z");
            }
        }
    }
    ...
    void CameraShake::Shake(const float& deltaTime)
    {
        ...
        else
        {
            ...
            // NOTE: Camera does not reset back to original position. It still retains ztranslation, and xRotation values from shake. Need a
            // better way of storing the original positions.
